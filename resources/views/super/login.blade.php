...
<head>
    ...
    <title>Staff sign in — Platform</title>
    @vite(['resources/css/app.css', 'resources/js/app.js'])
</head>
<body class="flex h-full items-center justify-center bg-slate-100 text-slate-900 antialiased">
    <div class="w-full max-w-sm">
        <h1 class="mb-1 text-xl font-semibold">Platform staff</h1>
        <p class="mb-6 text-sm text-slate-500">Sign in to manage plans and shops.</p>

        <form method="POST" action="{{ route('super.login.store') }}" class="space-y-4 rounded-xl bg-white p-6 shadow-sm">
            @csrf
